...
**Narrow decode (privacy boundary).** Each matching line is parsed through a **narrow decoder** that
declares only the metadata and usage fields listed above (`message.id`/`message.model`/`message.usage`,
`requestId`, `cwd`, `timestamp`, `uuid`). Prompts, assistant/tool message content, attachments, and
file-history snapshots are **not decoded into retained objects, written to the ledger, or included in
reports/diagnostics.**
...
- `~/Library/Application Support/AIPetUsage/claude-statusline.json` — written by this repo's own hook, `Scripts/claude-statusline-hook.sh` (install via `settings.json → statusLine`; see the README's "Claude Code official limits" section). The hook persists a **frozen allowlist only**: `{"schema_version":1,"captured_at":"<UTC ISO8601>","model":{"id","display_name"},"rate_limits":{"five_hour":{"used_percentage","resets_at"},"seven_day":{…}}}` — everything else in the payload (session id, transcript path, cwd, unknown fields at any level) is discarded, and a payload without a usable window never overwrites the last good file (staleness decays via mtime, below). Users with an existing custom statusline keep it intact via wrapper mode (`--wrap /path/to/their-command`): the hook lands the same file, then forwards the original stdin/stdout/stderr/exit code to their command untouched.
...
Most app-owned data lives in `~/Library/Application Support/AIPetUsage/`:
...
| `limits-state.json` | folded rate-limit windows (monotonic guard state) + plan/tier labels |
...
| `pet-state.json` | fullness / XP / feeding state |
| `refresh.lock` | flock file serializing app↔CLI write phases |

**Local paths persist on disk (but are redacted in what you share).** A ledger event's `projectId` is the
source's project identifier — when the source provides a local cwd, that is a **local path** — and each
event also records its `sourcePath` (the log file it was read from). `scan-state.json` is keyed by the
**full log-file paths** it has scanned and may hold parse context such as the last `cwd`. These are used
only for stable grouping / incremental resume. The UI and HTML reports display only **redacted project
names** (basename), never the full path — but the raw `ledger.jsonl` / `scan-state.json` on disk do contain
paths, so **sharing those files is not as safe as sharing an HTML report or `aipet diag`.**

**Not all state is in that folder.** Update-check preferences (auto-check toggle, last-check time, skipped
version) live in macOS **UserDefaults**; the optional scheduled export writes a **LaunchAgent** under
`~/Library/LaunchAgents/` and reports to the folder you chose; launch-at-login is system-managed.

**Resetting / deleting local data.** There is no one-click "erase everything" control today (Settings →
...
- If you set up the optional scheduled export, also remove its LaunchAgent
  (`~/Library/LaunchAgents/dev.aipetusage.app.report.plist`) and turn off launch-at-login, or a background
  run can recreate app data before you relaunch.
...
Reports are static, self-contained local HTML (embedded CSS, no external requests, no JavaScript required).
The report is a **sink-side fail-closed** artifact: project names are reduced to a **basename** (a
path-shaped or missing name is redacted, never emitted as a full path), and data-quality notes are emitted
as **closed-vocabulary** messages (raw parser-error text is never rendered). Reports contain project
**names**, aggregate token counts, costs, model IDs, limit percentages / reset times / confidence, activity
timeline + streak, and an optional pet summary — but never full local paths, prompts, or message contents.
